...
export async function GET(req: NextRequest) {
  try {
    ...
    const promptText = `You’re an English language expert and an expert in child education. Generate a random spelling bee word for pronunciation suitable for a ${ageLevel} year old. Only one word is needed. Response should just be the word followed by a newline, then its pronunciation followed by a newline, and a sentence using the word.`;
